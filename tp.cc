...
NS_LOG_COMPONENT_DEFINE ("tp");

int NUM_NODES = 50;    // number of nodes in the network  
                // default is 50  
int NETWORK_X = 1000;   // X-size of network  
                // default is 100  
int NETWORK_Y = 1000;   // Y-size of network  
                // default is 100  
...
double CLUSTER_PERCENT = 0.1; //if we change this make changes in threshold formulae and wherever we have put "20"  
...
printf("The modified simulation was able to remain viable for %d rounds\n", modification/TRIALS);
   ...
   NodeContainer leachNodes;
   leachNodes.Create(NUM_NODES);
   
   //NodeContainer csmaNodes;
   //csmaNodes.Add (leachNodes.Get (1));
   //csmaNodes.Create (NUM_NODES);
   
   //création d'un point-to-point (les lignes grises)
   PointToPointHelper pointToPoint;
   pointToPoint.SetDeviceAttribute ("DataRate", StringValue ("5Mbps"));
   pointToPoint.SetChannelAttribute ("Delay", StringValue ("2ms"));

   NetDeviceContainer devices;
   //devices.Add(pointToPoint.Install(leachNodes.Get(1), leachNodes.Get(2)));
   for (int i=1; i<NUM_NODES; i++) {
   	if(i<10)
   		devices.Add(pointToPoint.Install(leachNodes.Get(1), leachNodes.Get(i+1)));
   	if(i>10 && i<20)
   		devices.Add(pointToPoint.Install(leachNodes.Get(11), leachNodes.Get(i+1)));
   	if(i>20 && i<30)
   		devices.Add(pointToPoint.Install(leachNodes.Get(21), leachNodes.Get(i+1)));
   	if(i>30 && i<40)
   		devices.Add(pointToPoint.Install(leachNodes.Get(31), leachNodes.Get(i+1)));
   	if(i>40 && i<49)
   		devices.Add(pointToPoint.Install(leachNodes.Get(41), leachNodes.Get(i+1)));	
   }
   
   InternetStackHelper stack;
   stack.Install (leachNodes);
   
   Ipv4AddressHelper address;  
   address.SetBase ("10.1.2.0", "255.255.255.0");
   Ipv4InterfaceContainer leachInterfaces;
   leachInterfaces = address.Assign (devices);
   
   Ipv4GlobalRoutingHelper::PopulateRoutingTables ();
   
   // Create a packet sink on the star "hub" to receive these packets
	uint16_t port = 50000;
	Address sinkLocalAddress(InetSocketAddress ("255.255.255.0", port));
	PacketSinkHelper sinkHelper ("ns3::TcpSocketFactory", sinkLocalAddress);
	ApplicationContainer sinkApp = sinkHelper.Install (leachNodes);
	...
	Ptr<Socket> localSocket = Socket::CreateSocket (leachNodes.Get (0), TcpSocketFactory::GetTypeId ());

MobilityHelper mobility;
Ptr<ListPositionAllocator> positionAlloc = CreateObject<ListPositionAllocator>();

for (int i=0; i<NUM_NODES; i++) {
	positionAlloc->Add(Vector(network[i].xLoc, network[i].yLoc, 0.0));
}
mobility.SetPositionAllocator(positionAlloc);
mobility.SetMobilityModel ("ns3::RandomWalk2dMobilityModel", "Bounds", RectangleValue(Rectangle (-NETWORK_X, NETWORK_X, -NETWORK_Y, NETWORK_Y)));
mobility.Install(leachNodes);
  
  AnimationInterface::SetConstantPosition(leachNodes.Get(1), NETWORK_X, NETWORK_Y); 
  AnimationInterface anim ("tp.xml"); // Mandatory
  
  for(int i=0; i<NUM_NODES; ++i) {
  	if (i%10 == 1)
		anim.UpdateNodeColor (leachNodes.Get (i), 0, 255, 0); // point de couleur vert
  }
  
  pointToPoint.EnablePcap ("second", devices.Get (1), true);
